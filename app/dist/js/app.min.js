...
angular.module("clipr.clipped",["ui.router","ui.bootstrap","ngAside"]).controller("ClipController",["$scope","Clips","$modal","Notes","AuthService","$aside","$cookies",function($scope,Clips,$modal,Notes,AuthService,$aside,$cookies){$scope.clips=Clips.clips,$scope.clipShow=!1,$scope.loadAllClips=function(){Clips.loadAllClips($cookies.get("clipr"))},$scope.logOut=function(){AuthService.logOut()},$scope.clipToggle=function(){$scope.clipShow===!1&&($scope.clipShow=!0),$scope.clipShow===!0&&($scope.clipShow=!1)},$scope.showModal=function(clipUrl,size){$scope.opts={size:size,backdrop:!0,backdropClick:!0,dialogFade:!1,keyboard:!0,templateUrl:"./clipSelect/clipSelectView.html",controller:ModalInstanceCtrl,resolve:{}},$scope.opts.resolve.item=function(){return angular.copy({clip:clipUrl})};var modalInstance=$modal.open($scope.opts);modalInstance.result.then(function(){},function(){console.log("Modal Closed")})},$scope.openAside=function(position){console.log("inside asiiiiideee"),$aside.open({templateUrl:"./Suggestions/categorySuggestionsView.html",placement:position,backdrop:!1,controller:function($scope,$modalInstance){$scope.ok=function(e){$modalInstance.close(),e.stopPropagation()},$scope.cancel=function(e){$modalInstance.dismiss(),e.stopPropagation()}}})}}]);var ModalInstanceCtrl=function($scope,$modalInstance,$modal,item,$sce,Notes){$scope.item=item,$scope.sceUrl=$sce.trustAsResourceUrl($scope.item.clip),$scope.notes=Notes.notesObj,$scope.ok=function(){$modalInstance.close()},$scope.cancel=function(){$modalInstance.dismiss("cancel")},$scope.save=function(userNotes){$scope.NoteAndUrl={note:userNotes,url:$scope.item.clip},console.log("Notes being passed to server",$scope.NoteAndUrl),Notes.addNotes($scope.NoteAndUrl)},$scope.display=function(){console.log("display function!!!"),Notes.loadNotes($scope.item.clip)}};angular.module("clipr.sidebar",["ui.router"]).controller("SidebarController",["$scope","Clips",function($scope,Clips){console.log("placeholder to make linter happy"),$scope.categories=Clips.clips,$scope.loadClipsByCategory=function(category){console.log("category",category),Clips.loadClipsByCategory(category)}}]),angular.module("clipr.services",["ngCookies"]).service("Session",function(){this.create=function(sessionId,userId){this.id=sessionId,this.userId=userId},this.destroy=function(){this.id=null,this.userId=null}}).factory("Clips",["$http",function($http){var clips={data:[],clips:[],categories:{}},loadClipsByCategory=function(topic){var categorizedClips=[];"all"===topic&&(clips.clips=clips.data);for(var x=0;x<clips.data.length;x++){var node=clips.data[x];console.log(node),node.category===topic&&categorizedClips.push(node)}clips.clips=categorizedClips},loadAllClips=function(cookie){return $http({method:"GET",url:"/loadAllClips",params:{cookie:cookie}}).then(function(response){clips.data=response.data,clips.clips=response.data;for(var x=0;x<response.data.length;x++){console.log(response.data);var clip=response.data[x];console.log("CLIPS",x),clips.categories[clip.category]?clips.categories[clip.category].push(clip):clips.categories[clip.category]=[clip],console.log("clips.categories",clips.categories)}})};return{loadClipsByCategory:loadClipsByCategory,loadAllClips:loadAllClips,clips:clips}}]).factory("Notes",["$http",function($http){var notesObj={data:[]},loadNotes=function(param){return $http({method:"GET",url:"/user/get/loadNotes",params:{url:param}}).then(function(response){notesObj.data=response.data,console.log(notesObj)})},addNotes=function(param){return $http({method:"POST",url:"/user/post/addNote",params:param}).then(function(response){console.log("factory response",response),notesObj.data.push(response.data),console.log("notesArr inside addNotes",notesObj)})};return{loadNotes:loadNotes,addNotes:addNotes,notesObj:notesObj}}]).factory("AuthService",["$http","Session","$cookies","$state",function($http,Session,$cookies,$state){var isAuthenticated=function(){return console.log("cookies are delish",$cookies.get("clipr")),$cookies.get("clipr")?!0:!1},logOut=function(){console.log("in logout yo"),$cookies.remove("clipr"),$state.go("landing")};return{isAuthenticated:isAuthenticated,logOut:logOut}}]),angular.module("cliprApp",["ui.router","ui.bootstrap","ngAnimate","ngTouch","clipr.services","clipr.clipped","clipr.sidebar","clipr.suggested","clipr.categories"]).run(function($rootScope,$state,AuthService){$rootScope.$on("$stateChangeStart",function(event,toState,toParams,fromState,fromParams){toState.authenticate&&!AuthService.isAuthenticated()&&($state.transitionTo("landing"),event.preventDefault())})}).controller("AppController",["$scope","$location",function($scope,$location){}]).config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise("/"),$stateProvider.state("landing",{url:"/landing",views:{main:{templateUrl:"Landing/landingView.html",controller:"AuthController"}}}).state("categories",{url:"/categories",views:{main:{templateUrl:"categories/categories.html",controller:"CategoryController"},"header@categories":{templateUrl:"header/header.html",controller:"ClipController"}}}).state("main",{authenticate:!0,url:"/clips",views:{main:{templateUrl:"Clips/clippedView.html",controller:"ClipController"},"header@categories":{templateUrl:"header/header.html",controller:"ClipController"}}})}]),angular.module("clipr.categories",[]).controller("CategoryController",["$scope","Clips","$cookies","$state",function($scope,Clips,$cookies,$state){$scope.categories=Clips.clips,$scope.loadClipsByCategory=function(category){Clips.loadClipsByCategory(category),$state.go("main")},$scope.loadAllClips=function(){Clips.loadAllClips($cookies.get("clipr"))},$scope.navToClips=function(){Clips.loadAllClips($cookies.get("clipr")),$state.go("main")},$scope.loadAllClips()}]),angular.module("clipr.suggested",["ui.router"]);
=======
angular.module("clipr.auth",[]).controller("AuthController",function($scope){console.log("hello")}),angular.module("clipr.clipped",["ui.router","ui.bootstrap","ngAside"]).controller("ClipController",["$scope","Clips","$modal","Notes","AuthService","$aside","Suggestions",function($scope,Clips,$modal,Notes,AuthService,$aside,Suggestions){$scope.clips=Clips.clips,$scope.clipShow=!1,$scope.loadAllClips=function(){Clips.loadAllClips()},$scope.loadAllClips(),$scope.logOut=function(){AuthService.logOut()},$scope.clipToggle=function(){$scope.clipShow===!1&&($scope.clipShow=!0),$scope.clipShow===!0&&($scope.clipShow=!1)},$scope.showModal=function(clipIndex,size){$scope.opts={size:size,backdrop:!0,backdropClick:!0,dialogFade:!1,keyboard:!0,templateUrl:"./clipSelect/clipSelectView.html",controller:ModalInstanceCtrl,resolve:{}},$scope.opts.resolve.item=function(){return angular.copy({clipUrl:$scope.clips.data[clipIndex].clipUrl})};var modalInstance=$modal.open($scope.opts);modalInstance.result.then(function(){},function(){console.log("Modal Closed")})},$scope.openAside=function(position){console.log("inside asiiiiideee"),$aside.open({templateUrl:"./Suggestions/categorySuggestionsView.html",placement:position,backdrop:!1,controller:function($scope,$modalInstance){$scope.ok=function(e){$modalInstance.close(),e.stopPropagation()},$scope.cancel=function(e){$modalInstance.dismiss(),e.stopPropagation()}}})}}]);var ModalInstanceCtrl=function($scope,$modalInstance,$modal,item,$sce,Notes,Suggestions){$scope.item=item,$scope.sceUrl=$sce.trustAsResourceUrl($scope.item.clipUrl),$scope.notes=Notes.notesObj,$scope.suggestions=Suggestions.content,$scope.ok=function(){$modalInstance.close()},$scope.cancel=function(){$modalInstance.dismiss("cancel")},$scope.save=function(userNotes){$scope.NoteAndUrl={note:userNotes,url:$scope.item.clipUrl},console.log("Notes being passed to server",$scope.NoteAndUrl),Notes.addNotes($scope.NoteAndUrl)},$scope.display=function(){console.log("display function!!!"),Notes.loadNotes($scope.item.clipUrl)},$scope.getRelated=function(){console.log("Inside of $scope.getRelated"),Suggestions.getContent($scope.item.clipUrl)}};angular.module("clipr.header",["ui.router"]),angular.module("clipr.sidebar",["ui.router"]).controller("SidebarController",["$scope","Clips",function($scope,Clips){console.log("placeholder to make linter happy"),$scope.loadClipsByCategory=function(category){Clips.loadClipsByCategory(category)}}]),angular.module("clipr.services",["ngCookies"]).service("Session",function(){this.create=function(sessionId,userId){this.id=sessionId,this.userId=userId},this.destroy=function(){this.id=null,this.userId=null}}).factory("Clips",["$http",function($http){var clips={data:null},loadClipsByCategory=function(category){return console.log("category",category),$http({method:"POST",url:"/loadClipsByCategory",params:{category:category}}).then(function(response){console.log("category response yo",response),clips.data=response.data})},loadAllClips=function(){return $http({method:"GET",url:"/loadAllClips"}).then(function(response){console.log("load all clips response",response.data),clips.data=response.data,console.log(clips)})};return{loadClipsByCategory:loadClipsByCategory,loadAllClips:loadAllClips,clips:clips}}]).factory("Notes",["$http",function($http){var notesObj={data:[]},loadNotes=function(param){return $http({method:"GET",url:"/user/get/loadNotes",params:{url:param}}).then(function(response){notesObj.data=response.data,console.log(notesObj)})},addNotes=function(param){return $http({method:"POST",url:"/user/post/addNote",params:param}).then(function(response){console.log("factory response",response),notesObj.data.push(response.data),console.log("notesArr inside addNotes",notesObj)})};return{loadNotes:loadNotes,addNotes:addNotes,notesObj:notesObj}}]).factory("AuthService",["$http","Session","$cookies","$state",function($http,Session,$cookies,$state){var isAuthenticated=function(){return console.log("cookies are delish",$cookies.get("clipr")),$cookies.get("clipr")?!0:!1},logOut=function(){console.log("in logout yo"),$cookies.remove("clipr"),$state.go("landing")};return{isAuthenticated:isAuthenticated,logOut:logOut}}]).factory("Suggestions",["$http",function($http){var content={data:null};console.log("Inside of Suggestions Factory!! ");var getContent=function(url){return console.log("URL BEING PASSED TO SERVER",url),$http({method:"GET",url:"/getSuggestions",params:{url:url}}).then(function(response){content.data=response.data,console.log("this is content.data ",content.data)})["catch"](function(err){err&&console.log("error inside getContent ",err)})};return{content:content,getContent:getContent}}]),angular.module("cliprApp",["ui.router","ui.bootstrap","ngAnimate","ngTouch","clipr.services","clipr.clipped","clipr.header","clipr.sidebar","clipr.suggested","clipr.auth"]).run(function($rootScope,$state,AuthService){$rootScope.$on("$stateChangeStart",function(event,toState,toParams,fromState,fromParams){toState.authenticate&&!AuthService.isAuthenticated()&&($state.transitionTo("landing"),event.preventDefault())})}).controller("AppController",["$scope","$location",function($scope,$location){}]).config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$stateProvider.state("landing",{url:"/landing",views:{main:{templateUrl:"Landing/landingView.html",controller:"AuthController"}}}).state("main",{authenticate:!0,url:"/clips",views:{main:{templateUrl:"Clips/clippedView.html",controller:"ClipController"},"header@main":{templateUrl:"Clips/headerView.html",controller:"HeaderController"},"sidebar@main":{templateUrl:"Clips/sidebarView.html",controller:"SidebarController"}}})}]),angular.module("clipr.suggested",["ui.router"]);
>>>>>>> [feat]: site suggestions
<<<<<<< 1b628c042560710334c09c00ddf40a632c89da64
>>>>>>> [fix]: fix merge conflicts
=======
=======
angular.module("clipr.auth",[]).controller("AuthController",function($scope){console.log("hello")}),angular.module("clipr.clipped",["ui.router","ui.bootstrap","ngAside"]).controller("ClipController",["$scope","Clips","$modal","Notes","AuthService","$aside","$cookies","Suggestions",function($scope,Clips,$modal,Notes,AuthService,$aside,$cookies,Suggestions){$scope.clips=Clips.clips,$scope.clipShow=!1,$scope.loadAllClips=function(){Clips.loadAllClips($cookies.get("clipr"))},$scope.loadAllClips(),$scope.logOut=function(){AuthService.logOut()},$scope.clipToggle=function(){$scope.clipShow===!1&&($scope.clipShow=!0),$scope.clipShow===!0&&($scope.clipShow=!1)},$scope.showModal=function(clipIndex,size){$scope.opts={size:size,backdrop:!0,backdropClick:!0,dialogFade:!1,keyboard:!0,templateUrl:"./clipSelect/clipSelectView.html",controller:ModalInstanceCtrl,resolve:{}},$scope.opts.resolve.item=function(){return angular.copy({clipUrl:$scope.clips.data[clipIndex].clipUrl,title:$scope.clips.data[clipIndex].title})};var modalInstance=$modal.open($scope.opts);modalInstance.result.then(function(){},function(){console.log("Modal Closed")})},$scope.openAside=function(position){console.log("inside asiiiiideee"),$aside.open({templateUrl:"./Suggestions/categorySuggestionsView.html",placement:position,backdrop:!1,controller:function($scope,$modalInstance){$scope.ok=function(e){$modalInstance.close(),e.stopPropagation()},$scope.cancel=function(e){$modalInstance.dismiss(),e.stopPropagation()}}})}}]);var ModalInstanceCtrl=function($scope,$modalInstance,$modal,item,$sce,Notes,Suggestions){$scope.item=item,$scope.sceUrl=$sce.trustAsResourceUrl($scope.item.clipUrl),$scope.notes=Notes.notesObj,$scope.suggestions=Suggestions.content,console.log("TITLE INSIDE ARTICLE",$scope.item.title),$scope.ok=function(){$modalInstance.close()},$scope.cancel=function(){$modalInstance.dismiss("cancel")},$scope.save=function(userNotes){$scope.NoteAndUrl={note:userNotes,url:$scope.item.clipUrl},console.log("Notes being passed to server",$scope.NoteAndUrl),Notes.addNotes($scope.NoteAndUrl)},$scope.display=function(){console.log("display function!!!"),Notes.loadNotes($scope.item.clipUrl)},$scope.getRelated=function(){Suggestions.getContent($scope.item.title)}};angular.module("clipr.header",["ui.router"]),angular.module("clipr.sidebar",["ui.router"]).controller("SidebarController",["$scope","Clips",function($scope,Clips){console.log("placeholder to make linter happy"),$scope.categories=Clips.clips,$scope.loadClipsByCategory=function(category){console.log("category",category),Clips.loadClipsByCategory(category)}}]),angular.module("clipr.services",["ngCookies"]).service("Session",function(){this.create=function(sessionId,userId){this.id=sessionId,this.userId=userId},this.destroy=function(){this.id=null,this.userId=null}}).factory("Clips",["$http",function($http){var clips={data:[],clips:[],categories:{}},loadClipsByCategory=function(topic){var categorizedClips=[];"all"===topic&&(clips.clips=clips.data);for(var x=0;x<clips.data.length;x++){var node=clips.data[x];console.log(node),node.category===topic&&categorizedClips.push(node)}clips.clips=categorizedClips},loadAllClips=function(cookie){return $http({method:"GET",url:"/loadAllClips",params:{cookie:cookie}}).then(function(response){console.log("load all clips response",response.data),clips.data=response.data,clips.clips=response.data;for(var x=0;x<response.data.length;x++){var clip=response.data[x];clip.category||(clip.category="Other"),clips.categories[clip.category]?clip.categories[clip.category].push(clip):(console.log("in !",clip.category),clips.categories[clip.category]=[clip])}console.log("clips.categories",clips.categories)})};return{loadClipsByCategory:loadClipsByCategory,loadAllClips:loadAllClips,clips:clips}}]).factory("Notes",["$http",function($http){var notesObj={data:[]},loadNotes=function(param){return $http({method:"GET",url:"/user/get/loadNotes",params:{url:param}}).then(function(response){notesObj.data=response.data,console.log(notesObj)})},addNotes=function(param){return $http({method:"POST",url:"/user/post/addNote",params:param}).then(function(response){console.log("factory response",response),notesObj.data.push(response.data),console.log("notesArr inside addNotes",notesObj)})};return{loadNotes:loadNotes,addNotes:addNotes,notesObj:notesObj}}]).factory("AuthService",["$http","Session","$cookies","$state",function($http,Session,$cookies,$state){var isAuthenticated=function(){return console.log("cookies are delish",$cookies.get("clipr")),$cookies.get("clipr")?!0:!1},logOut=function(){console.log("in logout yo"),$cookies.remove("clipr"),$state.go("landing")};return{isAuthenticated:isAuthenticated,logOut:logOut}}]).factory("Suggestions",["$http",function($http){var content={data:null},getContent=function(title){return console.log("URL BEING PASSED TO SERVER",title),$http({method:"GET",url:"/getSuggestions",params:{title:title}}).then(function(response){content.data=response.data,console.log("this is content.data ",content.data)})["catch"](function(err){err&&console.log("error inside getContent ",err)})};return{content:content,getContent:getContent}}]),angular.module("cliprApp",["ui.router","ui.bootstrap","ngAnimate","ngTouch","clipr.services","clipr.clipped","clipr.header","clipr.sidebar","clipr.suggested","clipr.auth"]).run(function($rootScope,$state,AuthService){$rootScope.$on("$stateChangeStart",function(event,toState,toParams,fromState,fromParams){toState.authenticate&&!AuthService.isAuthenticated()&&($state.transitionTo("landing"),event.preventDefault())})}).controller("AppController",["$scope","$location",function($scope,$location){}]).config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$stateProvider.state("landing",{url:"/landing",views:{main:{templateUrl:"Landing/landingView.html",controller:"AuthController"}}}).state("main",{authenticate:!0,url:"/clips",views:{main:{templateUrl:"Clips/clippedView.html",controller:"ClipController"},"header@main":{templateUrl:"Clips/headerView.html",controller:"HeaderController"},"sidebar@main":{templateUrl:"Clips/sidebarView.html",controller:"SidebarController"}}})}]),angular.module("clipr.suggested",["ui.router"]);
>>>>>>> [feat]: get back suggestions  from AlchemyAPI
<<<<<<< 513d17d38da1d7d3b5d8f062a63eb8231bffdadf
>>>>>>> [feat]: get back suggestions  from AlchemyAPI
=======
=======
angular.module("clipr.auth",[]).controller("AuthController",function($scope){console.log("hello")}),angular.module("clipr.clipped",["ui.router","ui.bootstrap","ngAside"]).controller("ClipController",["$scope","Clips","$modal","Notes","AuthService","$aside","$cookies","Suggestions",function($scope,Clips,$modal,Notes,AuthService,$aside,$cookies,Suggestions){$scope.clips=Clips.clips,$scope.clipShow=!1,$scope.loadAllClips=function(){Clips.loadAllClips($cookies.get("clipr"))},$scope.loadAllClips(),$scope.logOut=function(){AuthService.logOut()},$scope.clipToggle=function(){$scope.clipShow===!1&&($scope.clipShow=!0),$scope.clipShow===!0&&($scope.clipShow=!1)},$scope.showModal=function(clipIndex,size){$scope.opts={size:size,backdrop:!0,backdropClick:!0,dialogFade:!1,keyboard:!0,templateUrl:"./clipSelect/clipSelectView.html",controller:ModalInstanceCtrl,resolve:{}},$scope.opts.resolve.item=function(){return angular.copy({clipUrl:$scope.clips.data[clipIndex].clipUrl,title:$scope.clips.data[clipIndex].title})};var modalInstance=$modal.open($scope.opts);modalInstance.result.then(function(){},function(){console.log("Modal Closed")})},$scope.openAside=function(position){console.log("inside asiiiiideee"),$aside.open({templateUrl:"./Suggestions/categorySuggestionsView.html",placement:position,backdrop:!1,controller:function($scope,$modalInstance){$scope.ok=function(e){$modalInstance.close(),e.stopPropagation()},$scope.cancel=function(e){$modalInstance.dismiss(),e.stopPropagation()}}})}}]);var ModalInstanceCtrl=function($scope,$modalInstance,$modal,item,$sce,Notes,Suggestions){$scope.item=item,$scope.sceUrl=$sce.trustAsResourceUrl($scope.item.clipUrl),$scope.sites=!1,$scope.suggestions=Suggestions.content.data,$scope.ok=function(){$modalInstance.close()},$scope.cancel=function(){$modalInstance.dismiss("cancel")},$scope.save=function(userNotes){$scope.NoteAndUrl={note:userNotes,url:$scope.item.clipUrl},console.log("Notes being passed to server",$scope.NoteAndUrl),Notes.addNotes($scope.NoteAndUrl)},$scope.displaySuggestions=function(){console.log("display function!!!"),$scope.sites=!0},$scope.getRelated=function(){Suggestions.getContent($scope.item.title),console.log($scope.suggestions)}};angular.module("clipr.header",["ui.router"]),angular.module("clipr.sidebar",["ui.router"]).controller("SidebarController",["$scope","Clips",function($scope,Clips){console.log("placeholder to make linter happy"),$scope.categories=Clips.clips,$scope.loadClipsByCategory=function(category){console.log("category",category),Clips.loadClipsByCategory(category)}}]),angular.module("clipr.services",["ngCookies"]).service("Session",function(){this.create=function(sessionId,userId){this.id=sessionId,this.userId=userId},this.destroy=function(){this.id=null,this.userId=null}}).factory("Clips",["$http",function($http){var clips={data:[],clips:[],categories:{}},loadClipsByCategory=function(topic){var categorizedClips=[];"all"===topic&&(clips.clips=clips.data);for(var x=0;x<clips.data.length;x++){var node=clips.data[x];console.log(node),node.category===topic&&categorizedClips.push(node)}clips.clips=categorizedClips},loadAllClips=function(cookie){return $http({method:"GET",url:"/loadAllClips",params:{cookie:cookie}}).then(function(response){console.log("load all clips response",response.data),clips.data=response.data,clips.clips=response.data;for(var x=0;x<response.data.length;x++){var clip=response.data[x];clip.category||(clip.category="Other"),clips.categories[clip.category]?clip.categories[clip.category].push(clip):(console.log("in !",clip.category),clips.categories[clip.category]=[clip])}console.log("clips.categories",clips.categories)})};return{loadClipsByCategory:loadClipsByCategory,loadAllClips:loadAllClips,clips:clips}}]).factory("Notes",["$http",function($http){var notesObj={data:[]},loadNotes=function(param){return $http({method:"GET",url:"/user/get/loadNotes",params:{url:param}}).then(function(response){notesObj.data=response.data,console.log(notesObj)})},addNotes=function(param){return $http({method:"POST",url:"/user/post/addNote",params:param}).then(function(response){console.log("factory response",response),notesObj.data.push(response.data),console.log("notesArr inside addNotes",notesObj)})};return{loadNotes:loadNotes,addNotes:addNotes,notesObj:notesObj}}]).factory("AuthService",["$http","Session","$cookies","$state",function($http,Session,$cookies,$state){var isAuthenticated=function(){return console.log("cookies are delish",$cookies.get("clipr")),$cookies.get("clipr")?!0:!1},logOut=function(){console.log("in logout yo"),$cookies.remove("clipr"),$state.go("landing")};return{isAuthenticated:isAuthenticated,logOut:logOut}}]).factory("Suggestions",["$http",function($http){var content={data:null},getContent=function(title){return console.log("URL BEING PASSED TO SERVER",title),$http({method:"GET",url:"/getSuggestions",params:{title:title}}).then(function(response){content.data=response.data,console.log("this is content.data ",content.data)})["catch"](function(err){err&&console.log("error inside getContent ",err)})};return{content:content,getContent:getContent}}]),angular.module("cliprApp",["ui.router","ui.bootstrap","ngAnimate","ngTouch","clipr.services","clipr.clipped","clipr.header","clipr.sidebar","clipr.suggested","clipr.auth"]).run(function($rootScope,$state,AuthService){$rootScope.$on("$stateChangeStart",function(event,toState,toParams,fromState,fromParams){toState.authenticate&&!AuthService.isAuthenticated()&&($state.transitionTo("landing"),event.preventDefault())})}).controller("AppController",["$scope","$location",function($scope,$location){}]).config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$stateProvider.state("landing",{url:"/landing",views:{main:{templateUrl:"Landing/landingView.html",controller:"AuthController"}}}).state("main",{authenticate:!0,url:"/clips",views:{main:{templateUrl:"Clips/clippedView.html",controller:"ClipController"},"header@main":{templateUrl:"Clips/headerView.html",controller:"HeaderController"},"sidebar@main":{templateUrl:"Clips/sidebarView.html",controller:"SidebarController"}}})}]),angular.module("clipr.suggested",["ui.router"]);
>>>>>>> [fix]: fetch suggestions from new faroo API
>>>>>>> [fix]: fetch suggestions from new faroo API
=======
angular.module("clipr.clipped",["ui.router","ui.bootstrap","ngAside"]).controller("ClipController",["$scope","Clips","$modal","Notes","AuthService","$aside","Suggestions","$cookies",function($scope,Clips,$modal,Notes,AuthService,$aside,Suggestions,$cookies){$scope.clips=Clips.clips,$scope.clipShow=!1,$scope.loadAllClips=function(){Clips.loadAllClips($cookies.get("clipr"))},$scope.logOut=function(){AuthService.logOut()},$scope.clipToggle=function(){$scope.clipShow===!1&&($scope.clipShow=!0),$scope.clipShow===!0&&($scope.clipShow=!1)},$scope.showModal=function(clipUrl,clipTitle,size){$scope.opts={size:size,backdrop:!0,backdropClick:!0,dialogFade:!1,keyboard:!0,templateUrl:"./clipSelect/clipSelectView.html",controller:ModalInstanceCtrl,resolve:{}},$scope.opts.resolve.item=function(){return angular.copy({clip:clipUrl,title:clipTitle})};var modalInstance=$modal.open($scope.opts);modalInstance.result.then(function(){},function(){console.log("Modal Closed")})},$scope.openAside=function(position){console.log("inside asiiiiideee"),$aside.open({templateUrl:"./Suggestions/categorySuggestionsView.html",placement:position,backdrop:!1,controller:function($scope,$modalInstance){$scope.ok=function(e){$modalInstance.close(),e.stopPropagation()},$scope.cancel=function(e){$modalInstance.dismiss(),e.stopPropagation()}}})}}]);var ModalInstanceCtrl=function($scope,$modalInstance,$modal,item,$sce,Notes,Suggestions){$scope.item=item,$scope.sceUrl=$sce.trustAsResourceUrl($scope.item.clip),$scope.sites=!1,$scope.suggestions=Suggestions.content.data,console.log("SCOPE ITEM INSIDE MODAL",$scope.item),$scope.ok=function(){$modalInstance.close()},$scope.cancel=function(){$modalInstance.dismiss("cancel")},$scope.save=function(userNotes){$scope.NoteAndUrl={note:userNotes,url:$scope.item.clip},console.log("Notes being passed to server",$scope.NoteAndUrl),Notes.addNotes($scope.NoteAndUrl)},$scope.displaySuggestions=function(){console.log("display function!!!"),Notes.loadNotes($scope.item.clip),$scope.sites=!0},$scope.getRelated=function(){console.log("TITLE TO PASS TO SUGGESTIONS",$scope.item.title),Suggestions.getContent($scope.item.title)}};angular.module("clipr.sidebar",["ui.router"]).controller("SidebarController",["$scope","Clips",function($scope,Clips){console.log("placeholder to make linter happy"),$scope.categories=Clips.clips,$scope.loadClipsByCategory=function(category){console.log("category",category),Clips.loadClipsByCategory(category)}}]),angular.module("clipr.services",["ngCookies"]).service("Session",function(){this.create=function(sessionId,userId){this.id=sessionId,this.userId=userId},this.destroy=function(){this.id=null,this.userId=null}}).factory("Clips",["$http",function($http){var clips={data:[],clips:[],categories:{}},loadClipsByCategory=function(topic){var categorizedClips=[];"all"===topic&&(clips.clips=clips.data);for(var x=0;x<clips.data.length;x++){var node=clips.data[x];console.log(node),node.category===topic&&categorizedClips.push(node)}clips.clips=categorizedClips},loadAllClips=function(cookie){return $http({method:"GET",url:"/loadAllClips",params:{cookie:cookie}}).then(function(response){clips.data=response.data,clips.clips=response.data;for(var x=0;x<response.data.length;x++){console.log(response.data);var clip=response.data[x];console.log("CLIPS",x),clips.categories[clip.category]?clips.categories[clip.category].push(clip):clips.categories[clip.category]=[clip],console.log("clips.categories",clips.categories)}})};return{loadClipsByCategory:loadClipsByCategory,loadAllClips:loadAllClips,clips:clips}}]).factory("Notes",["$http",function($http){var notesObj={data:[]},loadNotes=function(param){return $http({method:"GET",url:"/user/get/loadNotes",params:{url:param}}).then(function(response){notesObj.data=response.data,console.log(notesObj)})},addNotes=function(param){return $http({method:"POST",url:"/user/post/addNote",params:param}).then(function(response){console.log("factory response",response),notesObj.data.push(response.data),console.log("notesArr inside addNotes",notesObj)})};return{loadNotes:loadNotes,addNotes:addNotes,notesObj:notesObj}}]).factory("AuthService",["$http","Session","$cookies","$state",function($http,Session,$cookies,$state){var isAuthenticated=function(){return console.log("cookies are delish",$cookies.get("clipr")),$cookies.get("clipr")?!0:!1},logOut=function(){console.log("in logout yo"),$cookies.remove("clipr"),$state.go("landing")};return{isAuthenticated:isAuthenticated,logOut:logOut}}]).factory("Suggestions",["$http",function($http){var content={data:null},getContent=function(title){return console.log("URL BEING PASSED TO SERVER",title),$http({method:"GET",url:"/getSuggestions",params:{title:title}}).then(function(response){content.data=response.data,console.log("this is content.data ",content.data)})["catch"](function(err){err&&console.log("error inside getContent ",err)})};return{content:content,getContent:getContent}}]),angular.module("cliprApp",["ui.router","ui.bootstrap","ngAnimate","ngTouch","clipr.services","clipr.clipped","clipr.sidebar","clipr.suggested","clipr.categories"]).run(function($rootScope,$state,AuthService){$rootScope.$on("$stateChangeStart",function(event,toState,toParams,fromState,fromParams){toState.authenticate&&!AuthService.isAuthenticated()&&($state.transitionTo("landing"),event.preventDefault())})}).controller("AppController",["$scope","$location",function($scope,$location){}]).config(["$stateProvider","$urlRouterProvider",function($stateProvider,$urlRouterProvider){$urlRouterProvider.otherwise("/"),$stateProvider.state("landing",{url:"/landing",views:{main:{templateUrl:"Landing/landingView.html",controller:"AuthController"}}}).state("categories",{url:"/categories",views:{main:{templateUrl:"categories/categories.html",controller:"CategoryController"},"header@categories":{templateUrl:"header/header.html",controller:"ClipController"}}}).state("main",{authenticate:!0,url:"/clips",views:{main:{templateUrl:"Clips/clippedView.html",controller:"ClipController"},"header@categories":{templateUrl:"header/header.html",controller:"ClipController"}}})}]),angular.module("clipr.categories",[]).controller("CategoryController",["$scope","Clips","$cookies","$state",function($scope,Clips,$cookies,$state){$scope.categories=Clips.clips,$scope.loadClipsByCategory=function(category){Clips.loadClipsByCategory(category),$state.go("main")},$scope.loadAllClips=function(){Clips.loadAllClips($cookies.get("clipr"))},$scope.navToClips=function(){Clips.loadAllClips($cookies.get("clipr")),$state.go("main")},$scope.loadAllClips()}]),angular.module("clipr.suggested",["ui.router"]);
